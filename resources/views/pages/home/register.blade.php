...
<form method="POST" action="{{ $registerRoute }}" class="flex w-full flex-col items-center text-center">
	@csrf

	<h1 class="mb-6 text-2xl font-semibold">DAFTAR</h1>

	<div class="mb-5 w-full">
		<label for="name" class="mb-1 block text-left text-sm font-medium">Nama</label>
	...
	<div class="mb-5 flex w-full flex-col gap-1">
		<label for="password_confirmation" class="mb-1 block text-left text-sm font-medium">Konfirmasi Password</label>
		<div x-data="{ showPasswordConfirm: false }" class="relative">
			<input x-bind:type="showPasswordConfirm ? 'text' : 'password'" type="password" id="password_confirmation"
				name="password_confirmation" required
				class="w-full rounded-lg border border-gray-300 bg-white/50 px-3 py-2 text-sm text-gray-800 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-400/50"
				placeholder="Ulangi Password" />
			<button type="button" x-on:click="showPasswordConfirm = !showPasswordConfirm"
				class="absolute right-2.5 top-1/2 -translate-y-1/2 hover:cursor-pointer" aria-label="Show password">
				<i x-show="!showPasswordConfirm" class="fa-regular fa-eye"></i>
				<i x-show="showPasswordConfirm" class="fa-regular fa-eye-slash"></i>
			</button>
		</div>
	</div>
	...
	<p class="mt-4 text-sm text-gray-700">
		Sudah punya akun?
		<button class="text-blue-600 hover:underline" @click.prevent="tab = 'login'">Masuk</button>
	</p>
</form>
